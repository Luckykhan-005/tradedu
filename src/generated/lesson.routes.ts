...
import type { LessonHooks } from "./lesson.hooks"
...
/**
 * Set hooks for Lesson routes
 */
export function setLessonHooks(h: LessonHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["title", "content", "videoUrl", "duration", "order", "type", "moduleId", "createdAt", "updatedAt"] as const
const RELATION_FIELDS = ["module", "progress"] as const
...
/**
 * Create Lesson CRUD routes
 */
export function createLessonRoutes(): Hono {
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "Lesson not found" } }, 404)
      }

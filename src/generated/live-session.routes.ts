...
/**
 * Auto-generated LiveSession Routes
 *
...
import type { LiveSessionHooks } from "./live-session.hooks"
...
/**
 * Set hooks for LiveSession routes
 */
export function setLiveSessionHooks(h: LiveSessionHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["title", "description", "date", "duration", "meetLink", "courseId", "instructorId", "createdAt"] as const
const RELATION_FIELDS = ["course", "instructor"] as const
...
/**
 * Create LiveSession CRUD routes
 */
export function createLiveSessionRoutes(): Hono {
  ...
  router.get("/", async (c) => {
    try {
      ...
      console.error("[LiveSession] List error:", error)
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "LiveSession not found" } }, 404)
      }
  ...
  router.post("/", async (c) => {
    try {
      ...
      const item = await prisma.liveSession.create({
        data: picked.data,
      })
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      const item = await prisma.liveSession.update({
        where: { id },
        data: picked.data,
      })
      ...
      console.error("[LiveSession] Update error:", error)

...
/**
 * Auto-generated PasswordResetToken Routes
 *
...
import type { PasswordResetTokenHooks } from "./password-reset-token.hooks"
...
let hooks: PasswordResetTokenHooks = {}
...
/**
 * Set hooks for PasswordResetToken routes
 */
export function setPasswordResetTokenHooks(h: PasswordResetTokenHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["email", "token", "expiresAt", "used", "createdAt"] as const
...
/**
 * Create PasswordResetToken CRUD routes
 */
export function createPasswordResetTokenRoutes(): Hono {
  ...
  router.get("/", async (c) => {
    try {
      ...
      console.error("[PasswordResetToken] List error:", error)
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "PasswordResetToken not found" } }, 404)
      }

      return sendJson(c, { ok: true, data: item })
    } catch (error: any) {
      console.error("[PasswordResetToken] Get error:", error)
  ...
  router.post("/", async (c) => {
    try {
      ...
      const item = await prisma.passwordResetToken.create({
        data: picked.data,
      })
      ...
      console.error("[PasswordResetToken] Create error:", error)
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      const item = await prisma.passwordResetToken.update({
        where: { id },
        data: picked.data,
      })
      ...
      console.error("[PasswordResetToken] Update error:", error)
  ...
  router.delete("/:id", async (c) => {
    try {
      ...
      console.error("[PasswordResetToken] Delete error:", error)

...
import type { CourseHooks } from "./course.hooks"
...
/**
 * Set hooks for Course routes
 */
export function setCourseHooks(h: CourseHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["title", "description", "level", "thumbnail", "price", "isPublished", "duration", "rating", "studentCount", "createdAt", "updatedAt"] as const
const RELATION_FIELDS = ["modules", "enrollments", "sessions"] as const
...
/**
 * Create Course CRUD routes
 */
export function createCourseRoutes(): Hono {
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "Course not found" } }, 404)
      }

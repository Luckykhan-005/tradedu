...
import type { ModuleHooks } from "./module.hooks"
...
/**
 * Set hooks for Module routes
 */
export function setModuleHooks(h: ModuleHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["title", "description", "order", "courseId", "createdAt", "updatedAt"] as const
const RELATION_FIELDS = ["course", "lessons"] as const
...
/**
 * Create Module CRUD routes
 */
export function createModuleRoutes(): Hono {
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "Module not found" } }, 404)
      }

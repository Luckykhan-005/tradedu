...
/**
 * Auto-generated QuizResult Routes
 *
...
import type { QuizResultHooks } from "./quiz-result.hooks"
...
/**
 * Set hooks for QuizResult routes
 */
export function setQuizResultHooks(h: QuizResultHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["userId", "courseId", "score", "answers", "completedAt"] as const
const RELATION_FIELDS = ["user"] as const
...
/**
 * Create QuizResult CRUD routes
 */
export function createQuizResultRoutes(): Hono {
  ...
  router.get("/", async (c) => {
    try {
      ...
      console.error("[QuizResult] List error:", error)
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      const item = await prisma.quizResult.findUnique({
        where: { id },
      })

      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "QuizResult not found" } }, 404)
      }

      return sendJson(c, { ok: true, data: item })
    } catch (error: any) {
      console.error("[QuizResult] Get error:", error)
  ...
  router.post("/", async (c) => {
    try {
      ...
      console.error("[QuizResult] Create error:", error)
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      console.error("[QuizResult] Update error:", error)
  ...
  router.delete("/:id", async (c) => {
    try {
      ...
      console.error("[QuizResult] Delete error:", error)

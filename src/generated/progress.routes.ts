...
/**
 * Auto-generated Progress Routes
 *
...
/**
 * Set hooks for Progress routes
 */
export function setProgressHooks(h: ProgressHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["userId", "lessonId", "completed", "completedAt", "createdAt"] as const
const RELATION_FIELDS = ["user", "lesson"] as const
...
/**
 * Create Progress CRUD routes
 */
export function createProgressRoutes(): Hono {
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "Progress not found" } }, 404)
      }
  ...
  router.post("/", async (c) => {
    try {
      ...
      const item = await prisma.progress.create({
        data: picked.data,
      })
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      const item = await prisma.progress.update({
        where: { id },
        data: picked.data,
      })

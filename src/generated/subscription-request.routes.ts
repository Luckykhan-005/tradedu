...
/**
 * Auto-generated SubscriptionRequest Routes
 *
...
import type { SubscriptionRequestHooks } from "./subscription-request.hooks"
...
let hooks: SubscriptionRequestHooks = {}
...
/**
 * Set hooks for SubscriptionRequest routes
 */
export function setSubscriptionRequestHooks(h: SubscriptionRequestHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["name", "email", "phone", "city", "tradeExperience", "plan", "status", "receiptUrl", "adminNote", "createdAt", "updatedAt"] as const
...
/**
 * Create SubscriptionRequest CRUD routes
 */
export function createSubscriptionRequestRoutes(): Hono {
  ...
  router.get("/", async (c) => {
    try {
      ...
      console.error("[SubscriptionRequest] List error:", error)
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      const item = await prisma.subscriptionRequest.findUnique({
        where: { id },
      })

      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "SubscriptionRequest not found" } }, 404)
      }

      return sendJson(c, { ok: true, data: item })
    } catch (error: any) {
      console.error("[SubscriptionRequest] Get error:", error)
  ...
  router.post("/", async (c) => {
    try {
      ...
      const item = await prisma.subscriptionRequest.create({
        data: picked.data,
      })
      ...
      console.error("[SubscriptionRequest] Create error:", error)
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      const item = await prisma.subscriptionRequest.update({
        where: { id },
        data: picked.data,
      })
      ...
      console.error("[SubscriptionRequest] Update error:", error)
  ...
  router.delete("/:id", async (c) => {
    try {
      ...
      console.error("[SubscriptionRequest] Delete error:", error)

...
/**
 * Auto-generated Enrollment Routes
 *
...
import type { EnrollmentHooks } from "./enrollment.hooks"
...
/**
 * Set hooks for Enrollment routes
 */
export function setEnrollmentHooks(h: EnrollmentHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["userId", "courseId", "status", "enrolledAt", "completedAt"] as const
const RELATION_FIELDS = ["user", "course"] as const
...
export function createEnrollmentRoutes(): Hono {
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "Enrollment not found" } }, 404)
      }
